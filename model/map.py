...
class Map:
    ...
    def getSubTexture(self, surface: Widget, x: int, y: int):
    ...
    # TODO: Temporary for initial testing
    def draw(self, surface: Widget):
        sub_texture = self.getSubTexture(surface, 0, 0)
    ...
    def index_to_pixel(self, x: int, y: int) -> tuple[int, int]:
        ...
        """
        px = (self.x_offset + (self.x_margin + x) * self.pixel_density) / self.zoom
        py = (self.y_offset + (self.y_margin + y) * self.pixel_density) / self.zoom
